...
async function loadComponents(distDir, pathname, serverless, hasServerComponents, rootEnabled) {
    if (serverless) {
    ...
    const [DocumentMod, AppMod, ComponentMod, AppServerMod] = await Promise.all([
        Promise.resolve().then(()=>(0, _require).requirePage('/_document', distDir, serverless, rootEnabled)
        ),
        Promise.resolve().then(()=>(0, _require).requirePage('/_app', distDir, serverless, rootEnabled)
        ),
        Promise.resolve().then(()=>(0, _require).requirePage(pathname, distDir, serverless, rootEnabled)
        ),
        hasServerComponents ? Promise.resolve().then(()=>(0, _require).requirePage('/_app.server', distDir, serverless, rootEnabled)
        ) : null, 
    ]);
    ...
    if (hasServerComponents) {
        try {
            ...
            await (0, _require).requirePage((0, _normalizePagePath).normalizePagePath(pathname) + _constants.NEXT_CLIENT_SSR_ENTRY_SUFFIX, distDir, serverless, rootEnabled);
        } catch (_) {
    ...
    if (rootEnabled) {
        const pagePath = (0, _require).getPagePath(pathname, distDir, serverless, false, undefined, rootEnabled);
        isViewPath = !!(pagePath === null || pagePath === void 0 ? void 0 : pagePath.match(/server[/\\]views[/\\]/));
    }
    return {
        App,
        Document,
