...
var _viewRender = require("./view-render");
...
class NextNodeServer extends _baseServer.default {
    ...
    getViewPathsManifest() {
        if (this.nextConfig.experimental.viewsDir) {
            const viewPathsManifestPath = (0, _path).join(this.serverDistDir, _constants.VIEW_PATHS_MANIFEST);
    ...
    async renderHTML(req, res, pathname, query, renderOpts) {
        ...
        if (renderOpts.isViewPath) {
            return (0, _viewRender).renderToHTML(req.originalRequest, res.originalResponse, pathname, query, renderOpts);
        }
    ...
    getPagePath(pathname, locales) {
        return (0, _require).getPagePath(pathname, this.distDir, this._isLikeServerless, this.renderOpts.dev, locales, this.nextConfig.experimental.viewsDir);
    }
    async findPageComponents(pathname, query = {}, params = null) {
        ...
        for (const pagePath of paths){
            try {
                const components = await (0, _loadComponents).loadComponents(this.distDir, pagePath, !this.renderOpts.dev && this._isLikeServerless, this.renderOpts.serverComponents, this.nextConfig.experimental.viewsDir);
        ...
        return manifest.sortedMiddleware.map((page)=>{
            ...
            return {
                match: (0, _routeMatcher).getRouteMatcher((0, _routeRegex).getMiddlewareRegex(page, {
                    catchAll: (manifest === null || manifest === void 0 ? void 0 : (ref = manifest.middleware) === null || ref === void 0 ? void 0 : ref[page].name) === _constants1.MIDDLEWARE_FILENAME
                })),
                page
            };
        });
   ...
   */ getMiddlewareInfo(page) {
        ...
        let foundPage;
        try {
            foundPage = (0, _denormalizePagePath).denormalizePagePath((0, _normalizePagePath).normalizePagePath(page));
        } catch (err) {
            throw (0, _require).pageNotFoundError(page);
        }
        let pageInfo = manifest.middleware[foundPage];
        if (!pageInfo) {
            throw (0, _require).pageNotFoundError(foundPage);
        }
        var _env1, _wasm;
        return {
            name: pageInfo.name,
