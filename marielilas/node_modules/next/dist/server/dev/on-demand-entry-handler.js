...
function onDemandEntryHandler({ maxInactiveAge , multiCompiler , nextConfig , pagesBufferLength , pagesDir , rootDir , viewsDir , watcher  }) {
    ...
    function getPagePathsFromEntrypoints(type, entrypoints, root) {
        ...
        for (const entrypoint of entrypoints.values()){
            ...
            } else if (entrypoint.name === _constants.MIDDLEWARE_FILENAME) {
                pagePaths.push(`${type}/${entrypoint.name}`);
            }
        }
        return pagePaths;
    ...
    return {
        async ensurePage (page, clientOnly) {
            const pagePathData = await findPagePathData(rootDir, pagesDir, page, nextConfig.pageExtensions, viewsDir);
            ...
            const promises = (0, _entries).runDependingOnPageType({
                ...
                pageRuntime: (await (0, _entries).getPageStaticInfo(pagePathData.absolutePagePath, nextConfig)).runtime,
 ...
 */ async function findPagePathData(rootDir, pagesDir, page, extensions, viewsDir) {
    const normalizedPagePath = tryToNormalizePagePath(page);
    let pagePath = null;
    if (normalizedPagePath === _constants.MIDDLEWARE_FILE) {
    ...
    if (viewsDir) {
        pagePath = await (0, _findPageFile).findPageFile(viewsDir, normalizedPagePath, extensions);
        if (pagePath) {
            const pageUrl = (0, _ensureLeadingSlash).ensureLeadingSlash((0, _removePagePathTail).removePagePathTail((0, _normalizePathSep).normalizePathSep(pagePath), {
                keepIndex: true,
                extensions
            ...
            return {
                absolutePagePath: (0, _path).join(viewsDir, pagePath),
                bundlePath: _path.posix.join('views', (0, _normalizePagePath).normalizePagePath(pageUrl)),
                page: _path.posix.normalize(pageUrl)
            };
        }
    }
