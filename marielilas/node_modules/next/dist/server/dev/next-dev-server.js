...
var _viewPaths = require("../../shared/lib/router/utils/view-paths");
...
class DevServer extends _nextServer.default {
    ...
    constructor(options){
        ...
        const { pages: pagesDir , views: viewsDir  } = (0, _findPagesDir).findPagesDir(this.dir, this.nextConfig.experimental.viewsDir);
    ...
    async startWatcher() {
        ...
        return new Promise((resolve, reject)=>{
            ...
            const files1 = this.nextConfig.pageExtensions.map((extension)=>(0, _path).join(this.dir, `${_constants.MIDDLEWARE_FILENAME}.${extension}`)
            );
            ...
            wp.on('aggregated', async ()=>{
                ...
                for (const [fileName, meta] of knownFiles){
                    ...
                    const isViewPath = Boolean(this.viewsDir && (0, _normalizePathSep).normalizePathSep(fileName).startsWith((0, _normalizePathSep).normalizePathSep(this.viewsDir)));
                    const rootFile = (0, _absolutePathToPage).absolutePathToPage(fileName, {
                        pagesDir: this.dir,
                        extensions: this.nextConfig.pageExtensions
                    });
                    if (rootFile === _constants.MIDDLEWARE_FILE) {
                    ...
                    if (isViewPath) {
                        ...
                        pageName = (0, _viewPaths).normalizeViewPath(pageName);
           ...
           */ if (/[\\\\/]_middleware$/.test(pageName)) {
                        Log.error(`nested Middleware is not allowed (found pages${pageName}) - https://nextjs.org/docs/messages/nested-middleware`);
                        continue;
                    }
                    (0, _entries).invalidatePageRuntimeCache(fileName, meta.safeTime);
                    (0, _entries).runDependingOnPageType({
                        page: pageName,
                        pageRuntime: (await (0, _entries).getPageStaticInfo(fileName, this.nextConfig)).runtime,
                        onClient: ()=>{},
                        onServer: ()=>{},
                        onEdgeServer: ()=>{
                ...
                this.middleware = (0, _utils2).getSortedRoutes(routedMiddleware).map((page)=>({
                        match: (0, _routeMatcher).getRouteMatcher((0, _routeRegex).getMiddlewareRegex(page, {
    ...
    async prepare() {
        ...
        await (0, _verifyTypeScriptSetup).verifyTypeScriptSetup(this.dir, [
            this.pagesDir,
            this.viewsDir
        ].filter(Boolean), false, this.nextConfig);
    ...
    async hasPage(pathname) {
        ...
        if (normalizedPath === _constants.MIDDLEWARE_FILE) {
        ...
        if (this.viewsDir) {
            const pageFile = await (0, _findPageFile).findPageFile(this.viewsDir, normalizedPath, this.nextConfig.pageExtensions);
    ...
    async runMiddleware(params) {
        try {
            ...
            result === null || result === void 0 ? void 0 : result.waitUntil.catch((error)=>this.logErrorWithOriginalStack(error, 'unhandledRejection', 'edge-server')
            );
            return result;
        } catch (error) {
        ...
        if ((0, _isError).default(err) && err.stack) {
            try {
                ...
                if (frame.lineNumber && (frame === null || frame === void 0 ? void 0 : frame.file)) {
                    ...
                    const compilation = stats === 'server' ? (ref = this.hotReloader) === null || ref === void 0 ? void 0 : (ref6 = ref.serverStats) === null || ref6 === void 0 ? void 0 : ref6.compilation : (ref7 = this.hotReloader) === null || ref7 === void 0 ? void 0 : (ref8 = ref7.edgeServerStats) === null || ref8 === void 0 ? void 0 : ref8.compilation;
    ...
    async hasMiddleware(pathname, isSSR) {
        return this.hasPage(isSSR ? pathname : _constants.MIDDLEWARE_FILE);
    }
    async ensureMiddleware(pathname, isSSR) {
        return this.hotReloader.ensurePage(isSSR ? pathname : _constants.MIDDLEWARE_FILE);
    }
    generateRoutes() {
