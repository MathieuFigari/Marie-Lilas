...
class HotReloader {
    constructor(dir, { config , pagesDir , distDir , buildId , previewProps , rewrites , viewsDir  }){
    ...
    async start() {
            ...
            config1.entry = async (...args)=>{
                ...
                await Promise.all(Object.keys(_onDemandEntryHandler.entries).map(async (pageKey)=>{
                    ...
                    (0, _entries).runDependingOnPageType({
                        page,
                        pageRuntime: (await (0, _entries).getPageStaticInfo(absolutePagePath, this.config)).runtime,
                        onEdgeServer: ()=>{
                        ...
                        onServer: ()=>{
                            ...
                            entrypoints[bundlePath] = (0, _entries).finalizeEntrypoint({
                                ...
                                name: bundlePath,
                                isServerComponent,
                                value: this.viewsDir && bundlePath.startsWith('views/') ? (0, _entries).getViewsEntry({
                                    name: bundlePath,
                                    pagePath: (0, _path).join(_constants.VIEWS_DIR_ALIAS, (0, _path).relative(this.viewsDir, absolutePagePath)),
                                    viewsDir: this.viewsDir,
                                    pageExtensions: this.config.pageExtensions
                                }) : request
                            });
        ...
        const trackPageChanges = (pageHashMap, changedItems)=>(stats)=>{
                try {
                    stats.entrypoints.forEach((entry, key)=>{
                        if (key.startsWith('pages/') || key === _constants.MIDDLEWARE_FILENAME) {
        ...
        multiCompiler.hooks.done.tap('NextjsHotReloaderForServer', ()=>{
            ...
            const middlewareChanges = Array.from(changedEdgeServerPages).filter((name)=>name === _constants.MIDDLEWARE_FILENAME
            );
