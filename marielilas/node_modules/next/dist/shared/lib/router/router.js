...
class Router {
    ...
    async change(method, url, as, options, forcedScroll) {
        ...
        if (shouldResolveHref && pathname !== '/_error') {
            ...
            if (process.env.__NEXT_HAS_REWRITES && as.startsWith('/')) {
                const rewritesResult = (0, _resolveRewrites).default(addBasePath(addLocale(cleanedAs, nextState.locale), true), pages, rewrites, query, (p)=>resolveDynamicRoute(p, pages)
                , this.locales);
   ...
   */ async prefetch(url, asPath = url, options = {}) {
        ...
        if (process.env.__NEXT_HAS_REWRITES && asPath.startsWith('/')) {
            ...
            const rewritesResult = (0, _resolveRewrites).default(addBasePath(addLocale(asPath, this.locale), true), pages, rewrites, parsed.query, (p)=>resolveDynamicRoute(p, pages)
            , this.locales);
            if (rewritesResult.externalDest) {
                return;
            }
