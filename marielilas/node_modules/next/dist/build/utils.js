...
async function printTreeView(list, pageInfos, serverless, { distPath , buildId , pagesDir , pageExtensions , buildManifest , middlewareManifest , useStatic404 , gzipSize =true  }) {
    ...
    const hasCustomAppServer = await (0, _findPageFile).findPageFile(pagesDir, '/_app.server', pageExtensions);
    pageInfos.set('/404', {
    ...
    const pageList = list.slice().filter((e)=>!(e === '/_document' || e === '/_error' || !hasCustomApp && e === '/_app' || !hasCustomAppServer && e === '/_app.server')
...
async function copyTracedFiles(dir, distDir, pageKeys, tracingRoot, serverConfig, middlewareManifest) {
    ...
    for (const middleware of Object.values(middlewareManifest.middleware) || []){
        if (middleware.name === _constants.MIDDLEWARE_FILENAME) {
...
async function isEdgeRuntimeCompiled(compilation, module, config) {
    ...
    return (await (0, _entries).getPageStaticInfo(module.resource, config)).runtime === 'edge';
}
