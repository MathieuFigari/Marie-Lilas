...
async function build(dir, conf = null, reactProductionProfiling = false, debugOutput = false, runLint = true) {
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            const { pages: pagesDir , views: viewsDir  } = (0, _findPagesDir).findPagesDir(dir, config.experimental.viewsDir);
            ...
            const verifyResult = await nextBuildSpan.traceChild('verify-typescript-setup').traceAsyncFn(()=>(0, _verifyTypeScriptSetup).verifyTypeScriptSetup(dir, [
                    pagesDir,
                    viewsDir
                ].filter(Boolean), !ignoreTypeScriptErrors, config, cacheDir)
            );
            ...
            let viewPaths;
            if (viewsDir) {
                viewPaths = await nextBuildSpan.traceChild('collect-view-paths').traceAsyncFn(()=>(0, _recursiveReaddir).recursiveReadDir(viewsDir, new RegExp(`page\\.(?:${config.pageExtensions.join('|')})$`))
                );
            }
            const rootPaths = await (0, _flatReaddir).flatReaddir(dir, new RegExp(`^${_constants.MIDDLEWARE_FILENAME}\\.(?:${config.pageExtensions.join('|')})$`));
            ...
            if (viewPaths && viewsDir) {
                mappedViewPaths = nextBuildSpan.traceChild('create-views-mapping').traceFn(()=>(0, _entries).createPagesMapping({
                        pagePaths: viewPaths,
                        hasServerComponents,
                        isDev: false,
                        pagesType: 'views',
            ...
            if (mappedRootPaths === null || mappedRootPaths === void 0 ? void 0 : mappedRootPaths[_constants.MIDDLEWARE_FILE]) {
            ...
            await (async ()=>{
                ...
                await runWebpackSpan.traceAsyncFn(async ()=>{
                    ...
                    if (isLikeServerless) {
                        ...
                        _flightManifestPlugin.injectedClientEntries.clear();
                        serverResult = await (0, _compiler).runCompiler(configs[1], {
                            runWebpackSpan
                        });
                        edgeServerResult = configs[2] ? await (0, _compiler).runCompiler(configs[2], {
                        ...
                        if (!serverResult.errors.length && !(edgeServerResult === null || edgeServerResult === void 0 ? void 0 : edgeServerResult.errors.length)) {
                            _flightManifestPlugin.injectedClientEntries.forEach((value, key)=>{
            ...
            const { customAppGetInitialProps , namedExports , isNextImageImported: isNextImageImported1 , hasSsrAmpPages: hasSsrAmpPages1 , hasNonStaticErrorPage ,  } = await staticCheckSpan.traceAsyncFn(async ()=>{
                ...
                await Promise.all(pageKeys.map(async (page)=>{
                    ...
                    return checkPageSpan.traceAsyncFn(async ()=>{
                        ...
                        const pageRuntime = pagePath ? (await (0, _entries).getPageStaticInfo((0, _path).join(pagesDir, pagePath), config)).runtime : undefined;
            ...
            images.remotePatterns = (config === null || config === void 0 ? void 0 : (ref1 = config.experimental) === null || ref1 === void 0 ? void 0 : (ref2 = ref1.images) === null || ref2 === void 0 ? void 0 : ref2.remotePatterns) || [];
